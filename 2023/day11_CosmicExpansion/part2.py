...
class Universe:
    ...
    def get_distance(self, gal1, gal2):
        ...
        for e in self.expanded_row:
            if in_between(row1, row2, e): distance += (1000000 - 1)
        for e in self.expanded_col:
            if in_between(col1, col2, e): distance += (1000000 - 1)
        return distance

    def part2(self):
        total = 0
        for i in range(self.num_galaxies):
            for j in range(i + 1, self.num_galaxies):
                value = self.get_distance(self.galaxies[i], self.galaxies[j])
                total = value + total
        return total
